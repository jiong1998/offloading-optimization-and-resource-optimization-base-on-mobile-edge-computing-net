...
for i in range(epoch_test):
    ...
    while True:  # 每次循环执行一个动作
        ...
        # 执行，获得环境反馈
        s_, r, done, tc_wald, local_cost = env.step(action)
        step_list_test.append(tc_wald)
        local_costs.append(local_cost)
        # 将环境的反馈s,a,r,s_传入记忆库。用于经验回放
    ...
    plt.plot(range(200), step_list_test, color='skyblue', label='optimize_F_P')
    plt.plot(range(200), local_costs, color="yellow", label="local")
    plt.legend(loc="best")

    plt.xlabel("step_dqn_test")
    plt.ylabel("costs_test")
    plt.show()

plt.plot(range(epoch_test),local_costs_epoch, color='yellow', label='locals')
plt.plot(range(epoch_test),epoch_list_test,color='skyblue', label='optimize_F_P')
plt.xlabel("epoch_test")
plt.ylabel("costs_test")
plt.legend(loc="best")
plt.show()
