...
class environment(object):  # 奖励r由tc与tc-1比较大小得出。Hn：0.992912817258564,	0.992299817945962
    ...
    def Time(self, user_offrate):
        ...
        if (user_offrate!=self.judge).any():
            ...
            lumuda_user1=pow(np.sum(A_user1*np.sqrt(self.l1*user_offrate_user1*b_user1)/a_user1)/(self.p_tran[0][0]*A_user1-np.sum(b_user1*A_user1/a_user1)),2)
            lumuda_user2=pow(np.sum(A_user2*np.sqrt(self.l2*user_offrate_user2*b_user2)/a_user2)/(self.p_tran[0][1]*A_user2-np.sum(b_user2*A_user2/a_user2)),2)
